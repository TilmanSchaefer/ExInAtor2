...
tmp_folder=output_folder+"/temp_dir" 
...
if not os.path.isfile("table_kmer_counts.txt"):
	...
	if reg_regions=="YES":
		...
		subprocess.call("awk \'{print $4\"\t\"$2\"\t\"$3\"\t\"$1\"\t.\t\"$6}\' genes_n2.bed | sort -T ' + tmp_folder + ' -k1,1 -k2,2n > genes_n2_temp.bed", shell=True)
		subprocess.call("awk \'{print $4\"\t\"$2\"\t\"$3\"\t\"$1\"\t.\t\"$6}\' genes_n.bed | sort -T ' + tmp_folder + ' -k1,1 -k2,2n > genes_n_temp.bed", shell=True)
		subprocess.call("awk \'{print $4\"\t\"$2\"\t\"$3\"\t\"$1\"\t.\t\"$6}\' promoters_2.bed | sort -T ' + tmp_folder + ' -k1,1 -k2,2n > promoters_2_temp.bed", shell=True)
		subprocess.call("awk \'{print $4\"\t\"$2\"\t\"$3\"\t\"$1\"\t.\t\"$6}\' promoters.bed | sort -T ' + tmp_folder + ' -k1,1 -k2,2n > promoters_temp.bed", shell=True)
		...
		subprocess.call("sort -T ' + tmp_folder + ' -k1,1 -k2,2n -o genes_n2_temp.bed genes_n2_temp.bed", shell=True)
		subprocess.call("sort -T ' + tmp_folder + ' -k1,1 -k2,2n -o genes_n_temp.bed genes_n_temp.bed", shell=True)
		subprocess.call("mergeBed -i genes_n2_temp.bed -c 4,6 -o distinct | awk \'{print $4\"\t\"$2\"\t\"$3\"\t\"$1\"\t.\t\"$5}\' | sort -T ' + tmp_folder + ' -k1,1 -k2,2n> genes_n2.bed", shell=True)
		subprocess.call("mergeBed -i genes_n_temp.bed -c 4,6 -o distinct | awk \'{print $4\"\t\"$2\"\t\"$3\"\t\"$1\"\t.\t\"$5}\' | sort -T ' + tmp_folder + ' -k1,1 -k2,2n> genes_n.bed", shell=True)
					
		# Create splice sites
		
		subprocess.call("cat "+gtf_file+' | tr --delete \; | tr --delete \\" | grep -v \\# | grep -v chrM | awk \'$3==\"gene\"\' | awk \'{print $10\"\t\"$4-1\"\t\"$5\"\t\"$1\"\t.\t\"$7}\' | sort -T ' + tmp_folder + ' -k1,1 -k2,2n > genes_n_for_ss.bed', shell=True)
		subprocess.call("cat "+whole_genome+' | tr --delete \; | tr --delete \\" | grep -v \\# | grep -v chrM | awk \'$3==\"gene\"\' | awk \'{print $10\"\t\"$4-1\"\t\"$5\"\t\"$1\"\t.\t\"$7}\' | sort -T ' + tmp_folder + ' -k1,1 -k2,2n > genes_n2_for_ss.bed', shell=True)
		subprocess.call("mergeBed -i exons_n2.bed -c 4,6 -o distinct | sort -T ' + tmp_folder + ' -k1,1 -k2,2n > merged_exons_n2_for_ss.bed", shell=True)
		subprocess.call("mergeBed -i exons_n.bed -c 4,6 -o distinct | sort -T ' + tmp_folder + ' -k1,1 -k2,2n > merged_exons_n_for_ss.bed", shell=True)
		...
		subprocess.call("sort -T ' + tmp_folder + ' -k1,1 -k2,2n -o introns_n_for_ss_reduced.bed introns_n_for_ss_reduced.bed", shell=True)
		subprocess.call("sort -T ' + tmp_folder + ' -k1,1 -k2,2n -o introns_n2_for_ss_reduced.bed introns_n2_for_ss_reduced.bed", shell=True)
		...
		subprocess.call("sort -T ' + tmp_folder + ' -k1,1 -k2,2n -o merged_exons_n.bed merged_exons_n.bed", shell=True)
		subprocess.call("sort -T ' + tmp_folder + ' -k1,1 -k2,2n -o merged_exons_n2.bed merged_exons_n2.bed", shell=True)
...
if not os.path.isfile('table_probabilities_R.txt'):
	...
	slice, rem = divmod(len(input_file), cores)
	slices= numpy.multiply([s for s in range(0,cores+1)],slice)
	slices[cores] += rem
	sliced_input = []
	for n in range(0,cores):
		sliced_input.append(input_file[slices[n]:slices[n+1]])

	pool = mp.Pool(processes=cores)
	results = pool.map(simulate, sliced_input)
	for element in results:
		file2.write(element)
...
r_script_folder = "/".join(input_folder.split("/")[:-2])+"/"
subprocess.call('Rscript '+r_script_folder+'recurrence_script_statistics.r '+true_set, shell=True)
